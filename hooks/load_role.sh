...
#
# Convenience only. The supported path is `atk <repo> <role>`, which works with
...
ROLES_DIR="${ATK_ROLES_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/roles}"
...
case "$role" in
  *[!a-z0-9_-]*|-*) printf 'agentaker: no role file for "%s".\n' "$role"; exit 0 ;;
esac

if [ ! -f "$ROLES_DIR/$role.md" ]; then
  printf 'agentaker: no role file for "%s".\n' "$role"
  exit 0
fi
