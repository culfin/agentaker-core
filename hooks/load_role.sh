...
#
# Convenience only. The supported path is `wtr <repo> <role>`, which works with
...
case "$role" in
  *[!a-z0-9_-]*|-*) printf 'worktree-roles: no role file for "%s".\n' "$role"; exit 0 ;;
esac

if [ ! -f "$ROLES_DIR/$role.md" ]; then
  printf 'worktree-roles: no role file for "%s".\n' "$role"
  exit 0
fi
