...
#
# Convenience only. The supported path is `mdt <repo> <role>`, which works with
...
ROLES_DIR="${MDT_ROLES_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/roles}"
...
case "$role" in
  *[!a-z0-9_-]*|-*) printf 'mandate: no role file for "%s".\n' "$role"; exit 0 ;;
esac

if [ ! -f "$ROLES_DIR/$role.md" ]; then
  printf 'mandate: no role file for "%s".\n' "$role"
  exit 0
fi
