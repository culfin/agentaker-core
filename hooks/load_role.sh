...
#
# Convenience only. The supported path is `tender <repo> <role>`, which works with
...
ROLES_DIR="${TENDER_ROLES_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/roles}"
...
case "$role" in
  *[!a-z0-9_-]*|-*) printf 'treetender: no role file for "%s".\n' "$role"; exit 0 ;;
esac

if [ ! -f "$ROLES_DIR/$role.md" ]; then
  printf 'treetender: no role file for "%s".\n' "$role"
  exit 0
fi
