#!/usr/bin/env bash
# Named credentials (issue #5): ATK_CREDENTIAL=<label> selects an entry an
...
#
#   - Sourced by bin/atk, unconditionally, next to lib/tabs.sh and
...
#     than trusting what atk's process already saw.
#
# The guard at the bottom tells the two modes apart.
#
# Needs from bin/atk: nothing at source time.
...
#   keychain service: agentaker-cred-<label>
...
credential_service_name() { printf 'agentaker-cred-%s' "$1"; }
...
# shape as valid_role_name() in bin/atk: letters, digits, - and _, never
...
valid_credential_account() {
  case "$1" in
    ...
    LD_*|DYLD_*|TMUX*|ATK_*|RANDOM|SECONDS|LINENO) return 1 ;;
    *) return 0 ;;
  esac
}
...
#
# Counts the value's bytes instead of holding it: this runs in atk's own
# process, where a trace (`bash -x bin/atk ...`) would print any variable
...
# anywhere near it. $1 (ATK_HOME) and $2 (the label) both travel through
# this argv in plain sight; neither is a secret.
credential_wrapper_argv() {
  local atk_home=$1 label=$2
  shift 2
  ...
  LAUNCH_CMD=(bash "$atk_home/lib/credential.sh" "$label" "$@")
}

# Which label a worktree's session was started with -- the label, never the
# value -- because `atk restart` builds a new launch line long after
# ATK_CREDENTIAL has left anyone's environment. Without it a restart would
...
credential_record_file() {
  ...
  dir="${ATK_STATE_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/agentaker}/credentials"
...
credential_record() {
  ...
  if [ -n "$label" ]; then
    ...
    printf "atk: this worktree was last started with credential '%s' and this start has none -- a later restart of either window restores none\n" "$previous" >&2
    rm -f "$file"
  fi
}

# For `atk restart`: prints the recorded label (nothing if the session
...
credential_recorded() {
  ...
  file=$(credential_record_file "$1") || {
    printf 'atk: cannot tell whether %s was started with a credential -- not restarting without knowing\n' "$1" >&2
    return 1
  }
  [ -f "$file" ] || return 0
  label=$(head -n 1 "$file")
  if ! valid_credential_label "$label"; then
    printf 'atk: %s holds no valid credential label -- not restarting without it\n' "$file" >&2
    return 1
  fi
  if ! credential_available "$label"; then
    printf "atk: no readable credential named '%s' in the keychain -- this session was started with it, so it is not restarted without it. See docs/setup.md\n" "$label" >&2
    return 1
  fi
...
# agentaker-reviewer, exported as GH_TOKEN. Same route as a named credential
...
REVIEWER_TOKEN_WARNING='atk: no reviewer token found — approvals will fail. See docs/setup.md'
REVIEWER_TOKEN_PANE_WARNING='atk: the reviewer token could not be read here — starting without GH_TOKEN or GITHUB_TOKEN; approvals will fail. See docs/setup.md'
...
# trace (`bash -x bin/atk ...`) could print.
reviewer_token_available() {
  if command -v security >/dev/null 2>&1; then
    security find-generic-password -s agentaker-reviewer >/dev/null 2>&1 && return 0
  fi
  if command -v secret-tool >/dev/null 2>&1; then
    secret-tool lookup service agentaker-reviewer >/dev/null 2>&1 && return 0
  fi
  return 1
}
...
reviewer_token_wrap() {
  ...
  if reviewer_token_available; then
    ...
    LAUNCH_CMD=(bash "$atk_home/lib/credential.sh" --reviewer-token "$@")
  else
    printf '%s\n' "$REVIEWER_TOKEN_WARNING" >&2
  fi
}

# Direct mode only, inside the pane: the two lookups atk itself used to
...
# this path only runs when a agentaker-reviewer entry exists (two-account
...
reviewer_token_export() {
  local value=""
  if command -v security >/dev/null 2>&1; then
    value=$(security find-generic-password -s agentaker-reviewer -w 2>/dev/null) || value=""
  fi
  if [ -z "$value" ] && command -v secret-tool >/dev/null 2>&1; then
    value=$(secret-tool lookup service agentaker-reviewer 2>/dev/null) || value=""
  fi
  if [ -n "$value" ]; then
...
# credential_available() saw a moment earlier: that call ran in atk's own
...
credential_export_or_die() {
  ...
  valid_credential_label "$label" || {
    printf 'atk: "%s" is not a credential label -- refusing to start without it.\n' "$label" >&2
    return 1
  }
  service=$(credential_service_name "$label")
  account=$(credential_find_account "$service") || {
    printf 'atk: no credential named "%s" in the keychain -- refusing to start without it. See docs/setup.md\n' "$label" >&2
    return 1
  }
  value=$(credential_find_value "$service" "$account") || {
    printf 'atk: credential "%s" is unreadable -- refusing to start without it. See docs/setup.md\n' "$label" >&2
    return 1
  }
  if [ -z "$value" ]; then
    printf 'atk: credential "%s" is empty -- refusing to start without it. See docs/setup.md\n' "$label" >&2
    value=""
    return 1
  fi
  ...
  if ! export "$account=$value" 2>/dev/null || [ "${!account-}" != "$value" ]; then
    printf 'atk: credential "%s" could not be put into the environment as %s -- refusing to start without it.\n' "$label" "$account" >&2
    value=""
    return 1
  fi
  value=""
...
# Tells the two modes apart: sourced (bin/atk, and this file's own tests,
...
if [ "${BASH_SOURCE[0]:-}" = "$0" ]; then
  ...
  if ! credential_export_or_die "$credential_label"; then
    if [ -t 0 ]; then
      printf 'atk: this window stays open so the reason above can be read -- press Enter to close it.\n' >&2
      read -r _ || true
    fi
    exit 1
  fi
