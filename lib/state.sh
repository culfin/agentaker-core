#!/usr/bin/env bash
# The facts mdt can vouch for itself before a restart — git and GitHub state,
...
#
# Sourced by bin/mdt on demand, alongside lib/manage.sh, only for `restart` —
...
#
# Three-way discipline per line, the same one status_query() in bin/mdt uses:
...
collect_state() {
  ...
  {
    printf '## State (collected by mdt, not reported by the session)\n\n'
