...
echo "hook and mdt assemble the same text"
# The hook duplicates bin/mdt's assembly on purpose — mdt has no side-effect-free
...
"$PWD/bin/mdt" demo developer >/dev/null 2>&1
mdt_text=$(cat "$WT/.agents/context.md")
hook_text=$(CLAUDE_PROJECT_DIR="$WT" bash "$HOOK")
check "hook output matches mdt's context.md" "$mdt_text" "$hook_text"
rm -rf "$SANDBOX"
...
if [ "$(id -u)" -eq 0 ]; then
  ...
  out=$(CLAUDE_PROJECT_DIR="$TMP" MDT_ROLES_DIR="$BADROLES" bash "$HOOK" 2>&1)
  check "still exits 0" "0" "$?"
