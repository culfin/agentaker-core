...
echo "hook and tender assemble the same text"
# The hook duplicates bin/tender's assembly on purpose — tender has no side-effect-free
...
"$PWD/bin/tender" demo developer >/dev/null 2>&1
tender_text=$(cat "$WT/.agents/context.md")
hook_text=$(CLAUDE_PROJECT_DIR="$WT" bash "$HOOK")
check "hook output matches tender's context.md" "$tender_text" "$hook_text"
rm -rf "$SANDBOX"
...
if [ "$(id -u)" -eq 0 ]; then
  ...
  out=$(CLAUDE_PROJECT_DIR="$TMP" TENDER_ROLES_DIR="$BADROLES" bash "$HOOK" 2>&1)
  check "still exits 0" "0" "$?"
