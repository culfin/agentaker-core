...
echo "hook and wtr assemble the same text"
# The hook duplicates bin/wtr's assembly on purpose — wtr has no side-effect-free
...
"$PWD/bin/wtr" demo developer >/dev/null 2>&1
wtr_text=$(cat "$WT/.agents/context.md")
hook_text=$(CLAUDE_PROJECT_DIR="$WT" bash "$HOOK")
check "hook output matches wtr's context.md" "$wtr_text" "$hook_text"
rm -rf "$SANDBOX"
...
if [ "$(id -u)" -eq 0 ]; then
  ...
  out=$(CLAUDE_PROJECT_DIR="$TMP" WTR_ROLES_DIR="$BADROLES" bash "$HOOK" 2>&1)
  check "still exits 0" "0" "$?"
