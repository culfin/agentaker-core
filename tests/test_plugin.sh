...
echo "hook and atk assemble the same text"
# The hook duplicates bin/atk's assembly on purpose — atk has no side-effect-free
...
"$PWD/bin/atk" demo developer >/dev/null 2>&1
atk_text=$(cat "$WT/.agents/context.md")
hook_text=$(CLAUDE_PROJECT_DIR="$WT" bash "$HOOK")
check "hook output matches atk's context.md" "$atk_text" "$hook_text"
rm -rf "$SANDBOX"
...
if [ "$(id -u)" -eq 0 ]; then
  ...
  out=$(CLAUDE_PROJECT_DIR="$TMP" ATK_ROLES_DIR="$BADROLES" bash "$HOOK" 2>&1)
  check "still exits 0" "0" "$?"
