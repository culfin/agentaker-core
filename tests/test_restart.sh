#!/usr/bin/env bash
# `tender restart` against a stand-in, not a real coding agent — a real AI
...
# anything real — TENDER_DRY_RUN (set by make_sandbox) keeps cmd_start from
# touching tmux or a real coding agent at all.
make_worktree() {
  "$TENDER" demo developer "$1" >/dev/null 2>&1
}
...
# how tests/test_list_drop.sh fakes an open window for `tender drop` to see.
open_window() {
  ...
  tmux set-window-option -t "tender-demo:DEV·$suffix" automatic-rename off
  tmux set-window-option -t "tender-demo:DEV·$suffix" allow-rename off
}

echo "tender restart: happy path — hand over, replace, carry the state forward"
make_worktree happy
open_window happy "$STUB/fake-agent-responsive.sh"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo DEV·happy 2>&1)
check "exits 0" "0" "$?"
...
sleep 0.3
pane=$(tmux capture-pane -p -S - -t "tender-demo:DEV·happy")
...
check "the window name survived" "DEV·happy" \
  "$(tmux display-message -p -t "tender-demo:DEV·happy" '#{window_name}')"
check "the window index survived" "yes" \
  "$(tmux list-windows -t tender-demo -F '#{window_name}' | grep -qxF 'DEV·happy' && echo yes || echo no)"
check "automatic-rename is still off (Task 15 held)" "off" \
  "$(tmux show-window-options -t "tender-demo:DEV·happy" | awk '$1=="automatic-rename"{print $2}')"
check "allow-rename is still off" "off" \
  "$(tmux show-window-options -t "tender-demo:DEV·happy" | awk '$1=="allow-rename"{print $2}')"
check "remain-on-exit was put back (not left on)" "off" \
  "$(tmux show-window-options -t "tender-demo:DEV·happy" | awk '$1=="remain-on-exit"{print $2}')"

echo "tender restart: a half-typed line does not corrupt the handover phrase"
...
tmux send-keys -t "tender-demo:DEV·corrupt" "half-typed leftover"
TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo DEV·corrupt >/dev/null 2>&1
sleep 0.3
...
echo "tender restart: --fresh replaces without asking, and does not wait"
make_worktree fresh
...
start=$(date +%s)
out=$(TENDER_HANDOFF_TIMEOUT=999 TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo DEV·fresh --fresh 2>&1)
rc=$?
...
sleep 0.3
pane=$(tmux capture-pane -p -S - -t "tender-demo:DEV·fresh")
...
echo "tender restart: a session that never responds is not restarted"
...
out=$(TENDER_HANDOFF_TIMEOUT=2 TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo DEV·timeout 2>&1)
check "exits 1" "1" "$?"
...
contains "names --fresh as the explicit way out" "tender restart demo DEV·timeout --fresh" "$out"
...
check "the original process is still alive" "0" \
  "$(tmux display-message -p -t "tender-demo:DEV·timeout" '#{pane_dead}')"
...
lacks "and it was never replaced — nothing was printed to the pane" "Every role" \
  "$(tmux capture-pane -p -S - -t "tender-demo:DEV·timeout")"
check "remain-on-exit was put back after the abort" "off" \
  "$(tmux show-window-options -t "tender-demo:DEV·timeout" | awk '$1=="remain-on-exit"{print $2}')"

echo "tender restart: no running window — said so, nothing touched"
make_worktree missing
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo DEV·missing 2>&1)
check "exits 1" "1" "$?"
contains "says there's no running session" "no running session" "$out"

echo "tender restart: an unknown label is explained, not confused with a missing window"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo NOSUCH 2>&1)
check "exits 1" "1" "$?"
contains "says the label wasn't found" "no worktree named" "$out"

echo "tender restart: missing arguments print usage"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo 2>&1)
check "exits 2" "2" "$?"
contains "prints usage" "Usage:" "$out"

echo "tender restart: a respawn-pane failure is reported, not claimed as a restart"
...
out=$(PATH="$TMUXSTUB:$PATH" TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart demo DEV·respawnfail 2>&1)
...
check "remain-on-exit was put back even though it failed" "off" \
  "$(tmux show-window-options -t "tender-demo:DEV·respawnfail" | awk '$1=="remain-on-exit"{print $2}')"
rm -rf "$TMUXSTUB"

echo "tender restart --all: skips a dirty worktree, restarts a clean one"
...
# would just sit until TENDER_HANDOFF_TIMEOUT).
tmux kill-window -t "tender-demo:DEV·happy" 2>/dev/null
tmux kill-window -t "tender-demo:DEV·corrupt" 2>/dev/null
tmux kill-window -t "tender-demo:DEV·fresh" 2>/dev/null
tmux kill-window -t "tender-demo:DEV·timeout" 2>/dev/null
tmux kill-window -t "tender-demo:DEV·respawnfail" 2>/dev/null
...
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" restart --all 2>&1)
...
# TENDER_HANDOFF_TIMEOUT (default 60s) for a handover — with several running
