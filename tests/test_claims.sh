#!/usr/bin/env bash
# `mdt claims` / `mdt claim-release` — the release-then-reclaim mechanism
...
echo "mdt claims: arguments"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claims 2>&1); check "no repo exits 1" "1" "$?"
contains "explains what's missing" "give a repo" "$out"

out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claims no-such-repo 2>&1)
check "unknown repo exits 1" "1" "$?"
contains "names the missing repo" "is not a git repository" "$out"

echo "mdt claims: nothing claimed is an honest (none), not silence"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claims demo 2>&1)
check "exits 0" "0" "$?"
check "says none" "(none)" "$out"

out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claims demo --json 2>&1)
check "json form is an empty array, not a blank line" "[]" "$out"

echo "mdt claims: default threshold (no AGENTS.md) is 2 days"
...
echo "mdt claims: a claim past the default threshold is marked orphaned"
...
echo "mdt claims: --json carries the same facts, machine-readable"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claims demo --json 2>&1)
...
echo "mdt claims: an unreachable remote says so, and does not exit 0"
...
echo "mdt claims: one unreachable ref does not take the rest of the listing down"
...
echo "mdt claim-release: arguments"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo 2>&1)
check "missing claim exits 1" "1" "$?"
contains "shows usage" "usage: mdt claim-release" "$out"

out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo not-a-claim 2>&1)
check "a name that isn't issue-N/pr-N/N exits 1" "1" "$?"
contains "explains the accepted shapes" "issue-N" "$out"

out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo 999 2>&1)
check "nothing held exits 1" "1" "$?"
contains "says there is nothing to release" "nothing to release" "$out"

echo "mdt claim-release: refuses a claim younger than the threshold — this is the point, not a safety detail"
push_claim issue-5 "$FRESH"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo 5 2>&1)
check "exits 1" "1" "$?"
...
echo "mdt claim-release: releases and reclaims an orphaned claim, bare-number form"
push_claim issue-6 "$OLD"
before=$(held_sha issue-6)
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo 6 2>&1)
check "exits 0" "0" "$?"
...
echo "mdt claims: after a takeover, the same claim reads as fresh, not orphaned"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claims demo 2>&1)
...
echo "mdt claim-release: explicit issue-N / pr-N forms, and the PR comment goes through gh pr comment"
push_claim issue-7 "$OLD"
: > "$GH_CALLS"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo issue-7 2>&1)
...
push_claim pr-8 "$OLD"
: > "$GH_CALLS"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo pr-8 2>&1)
...
echo "mdt claim-release: a comment gh cannot post does not undo an already-reclaimed ref"
...
STUBEOF
chmod +x "$STUB/gh"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" claim-release demo 9 2>&1)
