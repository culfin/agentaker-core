#!/usr/bin/env bash
# `atk doctor` actually executes whatever launch_command() resolves to — the
...
# PATH, being Claude Code itself) — a bug that made `atk doctor` reach past
...
#!/usr/bin/env bash
# Reads its context argument and immediately prints the doctor marker atk
# looks for — stands in for a tool that received its role and said so.
cat "$1" >/dev/null
printf 'ATK-DOCTOR-OK\n'
STUBEOF
chmod +x "$STUB/okstub"
...
out=$(ATK_TOOLS_FILE="$TOOLS" PATH="$SAFE_PATH" "$ATK" doctor nosuchtool 2>&1)
check "exits 1" "1" "$?"
...
out=$(ATK_TOOLS_FILE="$TOOLS" PATH="$SAFE_PATH" "$ATK" doctor okgtool 2>&1)
check "exits 0" "0" "$?"
...
out=$(ATK_TOOLS_FILE="$TOOLS" PATH="$SAFE_PATH" ATK_DOCTOR_TIMEOUT=3 "$ATK" doctor silenttool 2>&1)
check "exits 1" "1" "$?"
...
out=$(ATK_TOOLS_FILE="$SANDBOX/no-tools-file" PATH="$SAFE_PATH" ATK_DOCTOR_TIMEOUT=3 "$ATK" doctor codex 2>&1)
check "exits 1" "1" "$?"
...
START=$(date +%s)
out=$(ATK_TOOLS_FILE="$TOOLS" PATH="$SAFE_PATH" ATK_DOCTOR_TIMEOUT=1 "$ATK" doctor hangtool 2>&1)
rc=$?
END=$(date +%s)
check "exits 1" "1" "$rc"
...
contains "offers a longer timeout for this tool" "ATK_DOCTOR_TIMEOUT=60 atk doctor hangtool" "$out"
...
out=$(ATK_TOOLS_FILE="$TOOLS" PATH="$SAFE_PATH" "$ATK" doctor notinstalled 2>&1)
check "exits 1" "1" "$?"
...
contains "says what to install and how to re-check" "  action: install no-such-binary, or put its directory on PATH, then run: atk doctor notinstalled" "$out"

echo "doctor: never touches a real project"
BEFORE=$(find "$SANDBOX/demo" -maxdepth 1 | sort)
ATK_TOOLS_FILE="$TOOLS" PATH="$SAFE_PATH" "$ATK" doctor okgtool >/dev/null 2>&1
...
out=$(ATK_TOOLS_FILE="$TOOLS" PATH="$SAFE_PATH" ATK_DOCTOR_TIMEOUT=1 "$ATK" doctor 2>&1)
