...
echo "atk status: arguments"
out=$(ATK_OWNER= "$ATK" status 2>&1); check "no owner exits 1" "1" "$?"
contains "no owner is explained" "owner" "$out"

echo "atk status: sections"
...
echo "atk status: sections, without a reviewer login"
out=$(PATH="$STUB:$PATH" ATK_REVIEWER= "$ATK" status someowner 2>&1)
...
contains "admits the queue is approximate" "ATK_REVIEWER" "$out"
...
echo "atk status: with a reviewer login"
: > "$GH_CALLS"
out=$(PATH="$STUB:$PATH" ATK_REVIEWER=somereviewer "$ATK" status someowner 2>&1)
calls=$(cat "$GH_CALLS")
contains "asks for that reviewer's queue" "review-requested somereviewer" "$calls"
lacks "does not fall back" "review none" "$calls"
lacks "no approximation notice" "ATK_REVIEWER" "$out"

echo "atk status: the blocked-issues note is section-specific, not unconditional"
# gh search issues has no dependency filter (see bin/atk's comment above the
...
chmod +x "$STUB/gh"
: > "$GH_CALLS"
out=$(PATH="$STUB:$PATH" ATK_REVIEWER=r "$ATK" status someowner 2>&1)
...
echo "atk status: a failed query is not an empty board"
cat > "$STUB/gh" <<'STUBEOF'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$GH_CALLS"
...
echo "atk attach: arguments"
out=$("$ATK" attach 2>&1); check "attach without repo exits 2" "2" "$?"

echo "atk attach: missing tmux is diagnosed as missing tmux"
...
out=$(PATH="${NOTMUX_PATH#:}" "$ATK" attach demo 2>&1)
check "exits 1" "1" "$?"
contains "names tmux" "tmux is not installed" "$out"
rm -rf "$NOTMUX_TMP"

echo "atk status: an issue on hold is not 'ready to pick up' (issue #6)"
...
JQEOF
  chmod +x "$JQSTUB/gh"
  out=$(PATH="$JQSTUB:$PATH" ATK_REVIEWER=r "$ATK" status someowner 2>&1)
