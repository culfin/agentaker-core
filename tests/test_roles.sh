...
contains "the repo name comes from the worktree atk named, not the branch" 'wt_name=$(basename "$(git rev-parse --show-toplevel)")' "$DEV"
...
contains "the human's way to free an issue is spelled out" "atk claim-release\` does not" "$DEV"
lacks "claim-release is not offered as the way to free an issue" "(\`atk claim-release\`, or removing the" "$DEV"
...
contains "the branch-name trap is named, with the suffix remedy" "atk acme developer laptop" "$FLOW"
