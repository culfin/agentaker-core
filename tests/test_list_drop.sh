...
make_sandbox
trap 'tmux kill-session -t tender-demo >/dev/null 2>&1; rm -rf "$SANDBOX"' EXIT

# A second repo, so `tender list` (no argument) has more than one heading to
# get right, and `tender list demo` has something to prove it excluded.
...
echo "tender list: nothing yet"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" list 2>&1)
contains "says so when a repo has no worktrees" "no worktrees under" "$out"

echo "tender list: shows what exists"
"$TENDER" demo developer >/dev/null 2>&1
"$TENDER" demo developer eyeoffice >/dev/null 2>&1
"$TENDER" other developer >/dev/null 2>&1
# A worktree whose directory name and branch differ, built directly like the
# drop-ambiguity fixtures below: tender's own worktrees always name the branch
...
# directory name and branch are genuinely different strings — on tender's own
...
contains "shows the branch tender created" "demo-developer-eyeoffice" "$out"
...
lacks "an idle worktree is not claimed to be running" "running" "$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" list demo 2>&1)"

echo "tender list: a repo argument filters to it"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" list demo 2>&1)
contains "shows the requested repo" "demo" "$out"
lacks "excludes the other repo" "other" "$out"

echo "tender list: --size actually measures"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" list demo --size 2>&1)
lacks "no longer says sizes are omitted" "sizes omitted" "$out"
contains "prints a real du size, not a placeholder" "K" "$out"

echo "tender list: an unknown repo is explained, not silently empty"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" list nosuchrepo 2>&1)
check "exits 1" "1" "$?"
contains "says it's not a git repository" "not a git repository" "$out"

echo "tender drop: refuses on uncommitted changes, and says how many"
...
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo DEV·eyeoffice 2>&1)
check "exits 1" "1" "$?"
...
echo "tender drop: refuses on commits with no remote, and names the branch"
...
echo "tender drop: refuses on an open tmux window, and names the session"
tmux new-session -d -s tender-demo -n "DEV·eyeoffice" sleep 60
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo DEV·eyeoffice 2>&1)
check "exits 1" "1" "$?"
contains "names the session to close" "tender-demo" "$out"
contains "says to close it first" "close it first" "$out"

echo "tender drop: --force overrides all three at once"
echo dirty >> "$SANDBOX/demo/.worktrees/demo-developer-eyeoffice/a.txt"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo DEV·eyeoffice --force 2>&1)
check "exits 0" "0" "$?"
...
out=$(tmux list-windows -t tender-demo 2>&1)
lacks "the tmux window is closed too" "DEV·eyeoffice" "$out"

echo "tender drop: a clean, pushed, closed worktree needs no --force"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo DEV 2>&1)
check "exits 0" "0" "$?"
contains "confirms the removal" "removed" "$out"
check "the worktree is actually gone, not just claimed to be" "no" \
  "$([ -d "$SANDBOX/demo/.worktrees/demo-developer" ] && echo yes || echo no)"

echo "tender drop: refuses an ambiguous label, and deletes neither"
...
# is documented to handle a worktree tender didn't create.
...
echo "tender drop: --force does not override an ambiguous label"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo DEV --force 2>&1)
...
echo "tender drop: the directory name resolves an ambiguous label unambiguously"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo demo-devops --force 2>&1)
check "exits 0" "0" "$?"
...
echo "tender drop: with the collision gone, the label alone works again"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo DEV --force 2>&1)
check "exits 0" "0" "$?"
contains "removes it" "removed" "$out"

echo "tender drop: unpushed commits are scoped to the worktree's own branch, not every local branch"
...
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo demo-clean 2>&1)
check "exits 0 — the unrelated branch's unpushed commit is not this worktree's problem" "0" "$?"
contains "confirms the removal" "removed" "$out"

echo "tender drop: the unpushed-commits message doesn't overstate what --force costs"
...
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo demo-unpushed 2>&1)
check "exits 1" "1" "$?"
...
echo "tender drop: an unknown label is explained, not confused with a missing repo"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop other NOSUCH 2>&1)
check "exits 1" "1" "$?"
contains "says the label wasn't found" "no worktree named" "$out"

echo "tender drop: missing arguments print usage"
out=$(TENDER_PROJECTS_DIR="$SANDBOX" "$TENDER" drop demo 2>&1)
check "exits 2" "2" "$?"
