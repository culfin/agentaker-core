...
MDT="$PWD/bin/mdt"
make_sandbox
trap 'tmux kill-session -t mdt-demo >/dev/null 2>&1; rm -rf "$SANDBOX"' EXIT

# A second repo, so `mdt list` (no argument) has more than one heading to
# get right, and `mdt list demo` has something to prove it excluded.
...
echo "mdt list: nothing yet"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" list 2>&1)
contains "says so when a repo has no worktrees" "no worktrees under" "$out"

echo "mdt list: shows what exists"
"$MDT" demo developer >/dev/null 2>&1
"$MDT" demo developer eyeoffice >/dev/null 2>&1
"$MDT" other developer >/dev/null 2>&1
# A worktree whose directory name and branch differ, built directly like the
# drop-ambiguity fixtures below: mdt's own worktrees always name the branch
...
# directory name and branch are genuinely different strings — on mdt's own
...
contains "shows the branch mdt created" "demo-developer-eyeoffice" "$out"
...
lacks "an idle worktree is not claimed to be running" "running" "$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" list demo 2>&1)"

echo "mdt list: a repo argument filters to it"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" list demo 2>&1)
contains "shows the requested repo" "demo" "$out"
lacks "excludes the other repo" "other" "$out"

echo "mdt list: --size actually measures"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" list demo --size 2>&1)
lacks "no longer says sizes are omitted" "sizes omitted" "$out"
contains "prints a real du size, not a placeholder" "K" "$out"

echo "mdt list: an unknown repo is explained, not silently empty"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" list nosuchrepo 2>&1)
check "exits 1" "1" "$?"
contains "says it's not a git repository" "not a git repository" "$out"

echo "mdt drop: refuses on uncommitted changes, and says how many"
...
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" drop demo DEV·eyeoffice 2>&1)
check "exits 1" "1" "$?"
...
echo "mdt drop: refuses on commits with no remote, and names the branch"
...
echo "mdt drop: refuses on an open tmux window, and names the session"
tmux new-session -d -s mdt-demo -n "DEV·eyeoffice" sleep 60
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" drop demo DEV·eyeoffice 2>&1)
check "exits 1" "1" "$?"
contains "names the session to close" "mdt-demo" "$out"
contains "says to close it first" "close it first" "$out"

echo "mdt drop: --force overrides all three at once"
echo dirty >> "$SANDBOX/demo/.worktrees/demo-developer-eyeoffice/a.txt"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" drop demo DEV·eyeoffice --force 2>&1)
check "exits 0" "0" "$?"
...
out=$(tmux list-windows -t mdt-demo 2>&1)
lacks "the tmux window is closed too" "DEV·eyeoffice" "$out"

echo "mdt drop: a clean, pushed, closed worktree needs no --force"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" drop demo DEV 2>&1)
check "exits 0" "0" "$?"
contains "confirms the removal" "removed" "$out"
check "the worktree is actually gone, not just claimed to be" "no" \
  "$([ -d "$SANDBOX/demo/.worktrees/demo-developer" ] && echo yes || echo no)"

echo "mdt drop: refuses an ambiguous label, and deletes neither"
...
# is documented to handle a worktree mdt didn't create.
...
echo "mdt drop: --force does not override an ambiguous label"
...
echo "mdt drop: the directory name resolves an ambiguous label unambiguously"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" drop demo demo-devops --force 2>&1)
check "exits 0" "0" "$?"
...
echo "mdt drop: with the collision gone, the label alone works again"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" drop demo DEV --force 2>&1)
check "exits 0" "0" "$?"
...
echo "mdt drop: missing arguments print usage"
out=$(MDT_PROJECTS_DIR="$SANDBOX" "$MDT" drop demo 2>&1)
check "exits 2" "2" "$?"
