...
echo "atk init: writes AGENTS.md"
...
echo "atk init: never overwrites an existing AGENTS.md"
...
echo "atk init: creates the worktrees"
...
echo "atk init: tells you what to do next"
contains "names the next command" "atk demo developer" "$out"
contains "mentions the ready label" "ready" "$out"

echo "atk init: offers to protect the production boundary"
STUB=$(mktemp -d)
cat > "$STUB/gh" <<'STUBEOF'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$GH_CALLS"
...
# The file-level `export ATK_NO_NETWORK=1` above must be overridden inline here
# (to empty) or this call would silently skip the network step being tested.
out=$(PATH="$STUB:$PATH" ATK_YES=1 ATK_NO_NETWORK= "$ATK" init demo 2>&1)
calls=$(cat "$GH_CALLS")
...
echo "atk init: an environment that already exists is reported as updated, not created"
: > "$GH_CALLS"
out=$(PATH="$STUB:$PATH" ATK_YES=1 ATK_NO_NETWORK= GH_ENV_EXISTS=1 "$ATK" init demo 2>&1)
...
# With ATK_NO_NETWORK the environment step must be skipped entirely.
: > "$GH_CALLS"
out=$(PATH="$STUB:$PATH" ATK_YES=1 ATK_NO_NETWORK=1 "$ATK" init demo 2>&1)
lacks "no network means no environment call" "environments/production" "$(cat "$GH_CALLS")"
rm -rf "$STUB"

echo "atk init: prints the detected trunk, not a hardcoded 'main'"
...
out=$(ATK_PROJECTS_DIR="$TRUNK_SANDBOX" ATK_YES=1 ATK_NO_NETWORK=1 "$ATK" init other-trunk 2>&1)
...
echo "atk init: flags override what trunk/reviewer/tests/boundary would otherwise propose"
...
out=$(ATK_PROJECTS_DIR="$FLAG_SANDBOX" ATK_NO_NETWORK=1 ATK_DRY_RUN=1 "$ATK" init flagged \
...
echo "atk init: --boundary '' writes an explicit 'deliberately none', not the generic example"
...
out=$(ATK_PROJECTS_DIR="$BOUND_SANDBOX" ATK_NO_NETWORK=1 ATK_DRY_RUN=1 "$ATK" init none --boundary '' --yes 2>&1)
...
echo "atk init: --yes without --boundary refuses rather than silently writing no boundary"
...
out=$(ATK_PROJECTS_DIR="$REFUSE_SANDBOX" ATK_NO_NETWORK=1 ATK_DRY_RUN=1 "$ATK" init norefuse --yes 2>&1)
...
echo "atk init: the existing env-var route (ATK_YES, no --yes flag) is untouched by the boundary guard"
...
out=$(ATK_PROJECTS_DIR="$LEGACY_SANDBOX" ATK_YES=1 ATK_NO_NETWORK=1 ATK_DRY_RUN=1 "$ATK" init legacy 2>&1)
...
echo "atk init: --no-agents-md / --no-labels / --no-environment / --no-worktrees skip individually"
...
out=$(ATK_PROJECTS_DIR="$SKIP_SANDBOX" ATK_DRY_RUN=1 "$ATK" init skipme \
...
echo "atk init: exit codes distinguish done from nothing-to-do across two runs"
...
out=$(ATK_PROJECTS_DIR="$IDEM_SANDBOX" ATK_NO_NETWORK=1 ATK_DRY_RUN=1 "$ATK" init idem --yes --boundary 'npm publish' 2>&1)
