...
echo "atk: usage"
out=$("$ATK" 2>&1); check "no arguments exits 2" "2" "$?"
contains "no arguments prints usage" "Usage:" "$out"
contains "documents ATK_DRY_RUN — the only way a user discovers it otherwise" "ATK_DRY_RUN" "$out"
contains "says a dry run still creates the worktree — it reads as side-effect free otherwise" "still created" "$out"
contains "documents ATK_YES — the only non-interactive route through init" "ATK_YES" "$out"
contains "documents ATK_NO_NETWORK" "ATK_NO_NETWORK" "$out"

echo "atk: starting a role"
...
echo "atk: the assembled context"
...
echo "atk: nothing extra in the working tree"
...
out=$(ATK_TOOL=codex "$ATK" demo developer 2>&1)
...
out=$(ATK_TOOL=nonesuch "$ATK" demo developer 2>&1)
contains "unknown tool explains itself" "context.md" "$out"

echo "atk: idempotence and instances"
"$ATK" demo developer >/dev/null 2>&1; check "re-run exits 0" "0" "$?"
"$ATK" demo developer a11y >/dev/null 2>&1
...
echo "atk: guards"
out=$("$ATK" nosuchrepo developer 2>&1); check "unknown repo exits 1" "1" "$?"
contains "unknown repo is explained" "not a git repository" "$out"
out=$("$ATK" demo nosuchrole 2>&1); check "unknown role exits 1" "1" "$?"
contains "unknown role is explained" "no role file" "$out"
out=$("$ATK" demo ../../etc/passwd 2>&1); check "path in role name exits 1" "1" "$?"
contains "rejected as a name, not as a missing file" "is not a role name" "$out"

echo "atk: a failed write is not reported as success"
if [ "$(id -u)" -eq 0 ]; then
  ...
  out=$("$ATK" demo maintainer 2>&1); check "unwritable worktree exits 1" "1" "$?"
...
out=$("$ATK" demo maintainer 2>&1)
contains "maintainer tags MNT" "demo · MNT" "$out"
out=$("$ATK" demo none 2>&1)
contains "none tags ---" "demo · ---" "$out"
out=$("$ATK" demo developer eyeoffice 2>&1)
