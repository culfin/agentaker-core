...
bin/atk-lint .          # checks the repository stays internally consistent
shellcheck -S warning bin/atk lib/init.sh lib/init_flags.sh lib/init_propose.sh lib/json.sh lib/manage.sh lib/tabs.sh lib/state.sh lib/tools.sh lib/doctor.sh lib/claims.sh lib/credential.sh lib/throttle.sh lib/status.sh lib/status_json.sh bin/atk-lint hooks/load_role.sh
```
...
so a green `tests/run.sh` and `bin/atk-lint` alone isn't green CI.
`bin/atk-lint` isn't a check of your working copy — it checks that examples
...
`gemini`, `cursor`, `copilot`, `anthropic`, or `openai`. `bin/atk-lint` greps
...
Add a line to your tools file (`${XDG_CONFIG_HOME:-$HOME/.config}/agentaker/tools`,
or `$ATK_TOOLS_FILE`) — no PR needed, no code change, no release to wait for.
See `docs/tools.md` for the format and the two-point contract a tool has to
meet, and run `atk doctor <tool>` to check it actually works before you rely
on it unattended.
...
is the whole reason issue #2 moved this out of `bin/atk` in the first place.
...
and `## Production boundary` at minimum (`bin/atk-lint` checks this), and any
role or subagent it names under `## Roles` / `## Subagents` must exist in this
repository.

## Every script under `bin/` and `lib/` stays under 450 lines

`bin/atk` holds the three everyday subcommands; `lib/init.sh` holds `init`,
...
ceiling; `lib/init_propose.sh` holds `init --propose --json`, the read-only
description of those steps a setup wizard shows (app issue #6), apart because
it must never write; `lib/json.sh` holds the one JSON string escaper that
file and `lib/claims.sh` share; `lib/manage.sh` holds `list`, `drop` and
`restart`; `lib/tabs.sh` holds the tab-legibility helpers (role tag, iTerm2
colour, the pane wrapper) that `bin/atk` and `lib/manage.sh` both call on
...
are on the hot path, so `bin/atk` sources them unconditionally rather than
...
two; `lib/doctor.sh` holds `atk doctor`,
...
`atk status` itself — the board's questions, the engine that asks gh and
reads its answer, and the text board — and `lib/status_json.sh` holds
`atk status --json` (app issue #10); both are sourced on demand for
`status` only, and the JSON board goes through the same questions and engine,
so the two boards cannot drift apart. `bin/atk-lint` checks all of them —
see the loop near the end of `bin/atk-lint` for the current list, which is
