...
shellcheck -S warning bin/tender lib/init.sh lib/init_flags.sh lib/init_propose.sh lib/json.sh lib/manage.sh lib/tabs.sh lib/state.sh lib/tools.sh lib/doctor.sh lib/claims.sh lib/credential.sh lib/throttle.sh bin/tender-lint hooks/load_role.sh
```
...
so a green `tests/run.sh` and `bin/tender-lint` alone isn't green CI.
`bin/tender-lint` isn't a check of your working copy — it checks that examples
...
`gemini`, `cursor`, `copilot`, `anthropic`, or `openai`. `bin/tender-lint` greps
...
Add a line to your tools file (`${XDG_CONFIG_HOME:-$HOME/.config}/treetender/tools`,
or `$TENDER_TOOLS_FILE`) — no PR needed, no code change, no release to wait for.
See `docs/tools.md` for the format and the two-point contract a tool has to
meet, and run `tender doctor <tool>` to check it actually works before you rely
on it unattended.
...
is the whole reason issue #2 moved this out of `bin/tender` in the first place.
...
and `## Production boundary` at minimum (`bin/tender-lint` checks this), and any
role or subagent it names under `## Roles` / `## Subagents` must exist in this
repository.

## Every script under `bin/` and `lib/` stays under 450 lines

`bin/tender` holds the three everyday subcommands; `lib/init.sh` holds `init`,
...
colour, the pane wrapper) that `bin/tender` and `lib/manage.sh` both call on
...
both `status` and every developer start ask it. `bin/tender-lint` checks all of them —
