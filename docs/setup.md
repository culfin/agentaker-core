...
## 1. Get the tool
    ...
    ln -s ~/.agentaker/bin/atk ~/bin/atk   # or wherever is actually on your PATH — see below
...
install), in which case the symlink is created, `atk` still says "command not
...
`atk` resolves its own symlink to find `roles/` next to the real install, so
it works wherever you link it from — move `~/.agentaker` later and the
...
`atk init` offers to do this; here it is by hand.
...
call. `atk init` checks first and tells you which one happened, but GitHub
...
`atk init` treats that failure as "create it by hand" and points back here;
...
`<repo>` below is resolved under `ATK_PROJECTS_DIR` (default `~/Projekte`) —
...
"not a git repository" against a path that doesn't exist:
    ...
    export ATK_PROJECTS_DIR=~/code   # only if your repos aren't under ~/Projekte
    atk init <repo>
...
name already carries "atk", repeating it would just cost characters:
...
session doesn't answer within `ATK_HANDOFF_TIMEOUT` seconds (default 60),
`atk` aborts rather than restarting — see [limits.md](limits.md) for why, and
for what `--fresh` costs you when you use it instead:

    atk restart myproject DEV --fresh   # replace without asking — loses state
    atk restart --all                   # every running session, across every project
...
N × `ATK_HANDOFF_TIMEOUT` if every one of them is wedged and times out
...
4. Store it in the OS keychain — never in a settings file:
       ...
       # macOS
       security add-generic-password -s agentaker-reviewer -a "$USER" -w '<token>'
       # Linux (libsecret)
       secret-tool store --label="agentaker reviewer" service agentaker-reviewer

   `atk` reads it back with `security find-generic-password -s
   agentaker-reviewer -w` (or the `secret-tool lookup` equivalent) — the
...
`atk <repo> reviewer` sets `GH_TOKEN` for that window only, and the token
never appears in any process's argv — not `atk`'s, not `tmux`'s, not the
agent's. `atk` itself only asks whether the entry exists (`security` without
...
same way, in `tests/test_reviewer_token.sh`. `atk restart` does the same.

If the token is missing, `atk` says so and starts anyway — you find out at
the first approval, not at session start:

    atk: no reviewer token found — approvals will fail. See docs/setup.md

If the entry existed when `atk` checked but cannot be read from inside the
...
inherited from you (most likely the developer's):
    ...
    atk: the reviewer token could not be read here — starting without GH_TOKEN or GITHUB_TOKEN; approvals will fail. See docs/setup.md
...
A separate mechanism from the reviewer token above, and stricter: `atk` never
...
this project) has stored an API key under a label in the OS keychain,
    ...
    ATK_CREDENTIAL=work atk myproject developer

picks that entry, puts it into the started agent's *environment*, and never
lets it touch a process's argv — not `atk`'s own, not `tmux`'s, not the
...
takes the route described here). `ATK_CREDENTIAL` avoids that: only the label travels through `atk`'s own argv
...
Storage contract — the app writes what `atk` reads, so agree it here:

    # macOS — -w last, with no value after it: security then prompts for it
    security add-generic-password -s agentaker-cred-work -a ANTHROPIC_API_KEY -w
    # Linux (libsecret) — prompts for the value as well
    secret-tool store --label="agentaker-cred-work" service agentaker-cred-work account ANTHROPIC_API_KEY
...
back by `security`, so `atk` gets it silently. An entry an app wrote
...
- **service**: `agentaker-cred-<label>` — `<label>` is what `ATK_CREDENTIAL`
...
- **account**: the environment variable name the started agent should see —
  `atk` never knows this in advance. It exports whatever account name the
...
warning: `atk` refuses to start the session at all.

    atk: no readable credential named 'work' in the keychain — refusing to start without it. See docs/setup.md
...
that. `ATK_CREDENTIAL` unset behaves exactly as it did before this mechanism
existed — nothing changes for a session that doesn't ask for a credential.

Both at once — `ATK_CREDENTIAL=work atk myproject reviewer` — works: the
...
`atk restart` keeps the credential. A start records the label — never
the value — once tmux has actually opened the window (a dry run records
nothing), under `${XDG_STATE_HOME:-~/.local/state}/agentaker/credentials/`,
...
The key is looked up twice: once by `atk` before anything starts, and
...
`LD_*`, `DYLD_*`, `TMUX*`, `ATK_*` and a few other shell variables.
