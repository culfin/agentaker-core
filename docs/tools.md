...
`agentaker` is vendor-neutral by design. Everything except `launch_command()`
(`lib/tools.sh`) works the same regardless of which coding agent you run: the
worktrees, the role files, `AGENTS.md`, the GitHub flow, `atk status`,
`atk attach`.

## The contract

A coding agent needs exactly two things to work with `agentaker`:
...
first, `atk` prints the path to `.agents/context.md` for you to paste as your
...
project:
    ...
    ${XDG_CONFIG_HOME:-$HOME/.config}/agentaker/tools

`$ATK_TOOLS_FILE` overrides the location.
...
- **name** — what you'd pass as `ATK_TOOL` (or the bare word, since it's the
  default lookup key: `ATK_TOOL=mytool atk myproject developer`).
- **launch command** — the argv that starts the tool, one line. `{context}`
  ...
  role — `atk` refuses to launch a line like that rather than doing it
...
- **status** — `verified:<date>` if you (or whoever wrote the line) actually
  ...
  say clearly. `atk doctor <tool>` is how you get to `verified:` truthfully;
  see below.
...
tool you asked for, means `atk` behaves exactly as it did before this table
...
real name, and would launch it with no role, silently — `atk` refuses that
...
## `atk doctor` — checking a tool actually works

    atk doctor            # every tool atk currently knows how to start
    atk doctor claude      # just this one
...
`ATK_DOCTOR_TIMEOUT` seconds (default 15) and reports whether the marker
arrived.
...
indefinitely. This matters more here than almost anywhere else in `agentaker`:
...
then how.
    ...
    hangtool:
      ...
      action: start it in a real session (ATK_TOOL=hangtool atk <repo> <role>) and check it names its role; or allow more time: ATK_DOCTOR_TIMEOUT=60 atk doctor hangtool

`human:` means only a person at this machine can close it; `fixable:` is
reserved for gaps `atk` could close itself — none today, since there is no
...
by hand or via `atk doctor`. For example:
...
the afternoon you spent trusting it. `atk` says this out loud, too — starting
...
| Tool | `ATK_TOOL` | How the role is passed | Status |
|---|---|---|---|
| Claude Code | `claude` | `--append-system-prompt-file` | **verified** 2026-09-20 |
| Codex CLI | `codex` | `--prompt-file` | **unverified** — please report |
| anything else, without a tools-file line | any | `atk` prints the path to `.agents/context.md`; paste it as your first message | works, manually |
...
file once `atk doctor codex` confirms it, or open a pull request that fixes
the built-in branch and updates this table's status — say which version you
tested against.

## Adding your tool to this table (optional)

Your tools-file line is all `agentaker` itself needs — nothing here requires
...
- Roles, worktrees, the GitHub flow, `atk status`, `atk attach` — all of it.
...
- The optional SessionStart hook in `hooks/`, packaged as a Claude Code plugin
  ...
  `atk` would have assembled, for a session started by hand
  (`cd <worktree> && claude`) rather than through `atk <repo> <role>`.

Neither is required. Without them the core is unchanged: a non-Claude session
still gets its worktree, its `AGENTS.md`, and — via `atk <repo> <role>` — its
