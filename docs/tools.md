...
`treetender` is vendor-neutral by design. Everything except `launch_command()`
(`lib/tools.sh`) works the same regardless of which coding agent you run: the
worktrees, the role files, `AGENTS.md`, the GitHub flow, `tender status`,
`tender attach`.

## The contract

A coding agent needs exactly two things to work with `treetender`:
...
first, `tender` prints the path to `.agents/context.md` for you to paste as your
...
project:
    ...
    ${XDG_CONFIG_HOME:-$HOME/.config}/treetender/tools

`$TENDER_TOOLS_FILE` overrides the location.
...
- **name** — what you'd pass as `TENDER_TOOL` (or the bare word, since it's the
  default lookup key: `TENDER_TOOL=mytool tender myproject developer`).
- **launch command** — the argv that starts the tool, one line. `{context}`
  ...
  role — `tender` refuses to launch a line like that rather than doing it
...
- **status** — `verified:<date>` if you (or whoever wrote the line) actually
  ...
  say clearly. `tender doctor <tool>` is how you get to `verified:` truthfully;
  see below.
...
tool you asked for, means `tender` behaves exactly as it did before this table
...
real name, and would launch it with no role, silently — `tender` refuses that
...
## `tender doctor` — checking a tool actually works

    tender doctor            # every tool tender currently knows how to start
    tender doctor claude      # just this one
...
`TENDER_DOCTOR_TIMEOUT` seconds (default 15) and reports whether the marker
arrived.
...
indefinitely. This matters more here than almost anywhere else in `treetender`:
...
by hand or via `tender doctor`. For example:
...
the afternoon you spent trusting it. `tender` says this out loud, too — starting
...
| Tool | `TENDER_TOOL` | How the role is passed | Status |
|---|---|---|---|
| Claude Code | `claude` | `--append-system-prompt-file` | **verified** 2026-09-20 |
| Codex CLI | `codex` | `--prompt-file` | **unverified** — please report |
| anything else, without a tools-file line | any | `tender` prints the path to `.agents/context.md`; paste it as your first message | works, manually |
...
file once `tender doctor codex` confirms it, or open a pull request that fixes
the built-in branch and updates this table's status — say which version you
tested against.

## Adding your tool to this table (optional)

Your tools-file line is all `treetender` itself needs — nothing here requires
...
- Roles, worktrees, the GitHub flow, `tender status`, `tender attach` — all of it.
...
- The optional SessionStart hook in `hooks/`, packaged as a Claude Code plugin
  ...
  `tender` would have assembled, for a session started by hand
  (`cd <worktree> && claude`) rather than through `tender <repo> <role>`.

Neither is required. Without them the core is unchanged: a non-Claude session
still gets its worktree, its `AGENTS.md`, and — via `tender <repo> <role>` — its
