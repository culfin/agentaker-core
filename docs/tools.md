...
`mandate` is vendor-neutral by design. Everything except one function in
`bin/mdt` — `launch_command()` — works the same regardless of which coding
agent you run: the worktrees, the role files, `AGENTS.md`, the GitHub flow,
`mdt status`, `mdt attach`.
...
has actually launched a session. The comment above it in `bin/mdt` says so:
...
One branch in `launch_command()` in `bin/mdt`, one row in this table. Nothing
