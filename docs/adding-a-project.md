...
`max-open-prs:` is optional and ships commented out, inside `<!-- -->` so
Markdown does not render it as a heading. Replace the comment with a plain
`max-open-prs: N` line to cap how many agent PRs this project lets pile up
before its developers stop opening new ones and work off review feedback
instead — `docs/limits.md`, "`max-open-prs` caps agent PRs", says what it
