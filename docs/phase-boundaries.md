...
| **Hand over in place** (`atk restart`) | written to `.agents/handoff.md`, then read into the new context | replaced in place (`tmux respawn-pane`) | same |
...
- **Subagent** — a bounded sub-task inside the current phase that would
  ...
  wide search, a narrow investigation. Agentaker's subagents under `agents/`
...
- **Hand over in place (`atk restart`)** — the phase is not done, but this
  *process* needs replacing: a context that has gotten stuck, a crashed pane,
  ...
  mechanised in `agentaker`: it sends the handover prompt, waits for
  `.agents/handoff.md` to appear, then replaces the process
  (`restart_window()` in `lib/manage.sh`) — see `roles/_base.md`, "Handing
  over," for the four-line shape the file itself takes. `atk restart <repo>
...
- **Hand off, portable** — the *destination* changes, not just the process: a
  ...
  of agentaker's machinery travels with it — no worktree, no `.agents/`
...
decision in this whole map." `agentaker` borrows the question, not the
...
portable handoff, a manual step outside anything `agentaker` tracks.
